...
from ls_pde import evolve_level_set

import warnings
warnings.simplefilter(action='ignore', category=FutureWarning)

# ---------------- CONFIG ----------------
TUMOR_DIR = Path("/home/zengy2/isilon/Isaac/MRI_data/Clinic/Simulation/data/meningioma_seg_ori")
BRAIN_DIR = Path("/home/zengy2/isilon/Isaac/MRI_data/Clinic/Simulation/data/brain_seg_ori")
...
def main():
    ...
    for C in cases:
        ...
        if S0.shape != S1.shape or S0.shape != brain0.shape:
            ...
            continue
        dataset.append(dict(S0=S0, S1=S1, brain=brain0, spacing=sp0, delta_t=C["delta_t"]))
    ...
    def objective(theta):
        ...
        for C in dataset:
            ...
            _, S1_pred = evolve_level_set(
                S0=C["S0"], brain=C["brain"], spacing=C["spacing"],
                t_final=float(t_final),
                v0=v0, alpha=alpha, sigma_mm=sigma_mm,
                a_parallel=a_par, a_perp=a_perp,
                reinit_every=10, P_mode="increasing",
                smooth_normals_sigma=smooth_nrm, cfl=0.9,
            )
            d  = dice(S1_pred, C["S1"])
